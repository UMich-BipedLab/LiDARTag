...
%% Mocap data
% LiDAR1
% L1_trans_mean = mean(lidar_pos(1, :, distance(estimation_indices,1):distance(estimation_indices,2)), 3) ./ 1000;
% L1_rot_mean = mean(lidar_rot(1, :, distance(estimation_indices,1):distance(estimation_indices,2)), 3);
...
% case 1: assume the mocap returns (R0, T0)
% where R0: R_local_from_global
H_W1T1 = constructHByRotationTranslation(T1_rotm', -T1_rotm' * T1_trans_mean');
H_W1L1 = constructHByRotationTranslation(L1_rotm', -L1_rotm' * L1_trans_mean');

% case 2: assume the mocap returns (R0, T0)
% where R0: R_global_from_local
% H_W1T1 = constructHByRotationTranslation(T1_rotm, -T1_rotm * T1_trans_mean');
% H_W1L1 = constructHByRotationTranslation(L1_rotm, -L1_rotm * L1_trans_mean');    
...
M_cur_trans = -M_H_L2T2(1:3,1:3)' * M_H_L2T2(1:3,4);
